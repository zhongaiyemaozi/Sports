...
extension CLPhotoRecorder {
    ...
    func switchCamera() {
        ...
        let recserPostion = position == AVCaptureDevicePosition.back ? AVCaptureDevicePosition.front : AVCaptureDevicePosition.back
        var device: AVCaptureDevice?
        let arr = AVCaptureDevice.devices()
        for obj in arr! {
            let captureDevice = obj as! AVCaptureDevice
            if captureDevice.position == recserPostion {
    ...
    func capture(completion:@escaping (_ comImage : UIImage) ->()) {
        ...
        captureStillImageOutput?.captureStillImageAsynchronously(from: connection, completionHandler: { (imageDataSampleBuffer, error) in
            ...
            if error == nil {
                ...
                let resultImage = self.cutImageWithSourceImage(image: image!)
                completion(resultImage)
                UIImageWriteToSavedPhotosAlbum(resultImage, self, #selector(self.image(image:didFinishSavingWithError:contextInfo:)), nil)
            }else {
                print(error ?? String())
            }
        })
...
extension CLPhotoRecorder {
    ...
    fileprivate func setupSession() {
        ...
        let arr = AVCaptureDevice.devices();
        for obj in arr! {
            let captureDevice: AVCaptureDevice = obj as! AVCaptureDevice
        ...
        preView?.layer.insertSublayer(captureVideoPreviewLayer!, at: 0)
        captureVideoPreviewLayer?.bounds = (preView?.bounds)!
        captureVideoPreviewLayer?.position = (preView?.center)!
