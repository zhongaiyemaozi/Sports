    ...
    fileprivate func generateChileConteoller(dict:[String:Any]) -> CLNavigationVC? {
        ...
        if let clsName = dict["clsName"] {
            ...
            if let classVC = NSClassFromString(className) as? CLBaseVC.Type {
                ...
                controller.tabBarItem.setTitleTextAttributes([NSForegroundColorAttributeName : tabBarColor], for: .normal)
                controller.tabBarItem.setTitleTextAttributes([NSForegroundColorAttributeName : tabBarColor], for: .selected)
